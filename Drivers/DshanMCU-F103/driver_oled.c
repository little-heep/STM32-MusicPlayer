...
void OLED_PrintElse(uint8_t x,uint8_t y,uint8_t content[][16],uint8_t count)
{
//	uint8_t g_chinese_fonts[9][16]=content;
	uint8_t page = y;
	uint8_t col  = x*8;
	...
	{
		int width=sizeof(content[i]);
		/* 设置位置: page,col */
    OLED_SetPosition(page, col);

		/* 发送16字节数据(汉字的上半部) */
    OLED_WriteNBytes((uint8_t*)&content[i][0],width);

		/* 设置位置: page+1,col */
    OLED_SetPosition(page+1, col);
		
		/* 发送16字节数据(汉字的下半部) */
    OLED_WriteNBytes((uint8_t*)&content[i+1][0], width);
